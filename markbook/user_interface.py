...
while quit == False:
    ...
    if commands == "home":
        ...
        if action == "1":
            book.add_class(Classroom(class_name = input("Class name: "), course_code = input("Course code: "), course_name = input("Course name: "), period = input("Period number: "), teacher_name = input("Teacher: ")))
            book.print_info()

        elif action == "2":
            book.remove_class(input("Name of class to delete: "))
            book.print_info()
        
        elif action == "3":
            book.tweak(input("Which class' information would you like to edit"), input("Which parameter would you like to edit"), input("New value"))
            book.print_info()

        elif action == "4":
            room = input("Which class would you like to see: ")
            book.classrooms[room].print_info()
            commands = "room" 
            page = book.classrooms[room]

    
    elif commands == "room":
        
        action = input("What would you like to do? \n Add student: 1 \n terminate student: 2 \n Edit student information: 3 \n View class assignments: 4 \n Quit: 5 \n" )

        if action == "1": 
            student = Student(first_name= input("First name: "), last_name= input("Last name: "), gender= input("Gender: "), image= input(), student_number= input(), grade= input(), email= input(), marks= input(), comments= input())
            page.add_student(student)
            page.print_info()

        elif action == "2":
            page.remove_student(input("First name: "), input("Last name: "))
            page.print_info()
